    ...
    <>
      ...
      <header className="md:hidden fixed top-0 left-0 w-full h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 z-40">
        ...
        <button
          ...
          className="text-slate-500 hover:text-slate-800 p-1 rounded-lg focus:outline-none"
        >
      ...
      {/* 2. Mobile Drawer */}
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 md:hidden">
          <div className="fixed top-0 left-0 w-64 h-full bg-white border-r border-slate-200 p-5 flex flex-col justify-between">
            ...
            <div className="space-y-4 pt-6 border-t border-slate-100">
              <div className="px-2">
                <p className="text-xs font-bold text-slate-850 truncate">{session?.name || "User"}</p>
                <p className="text-[10px] text-slate-500 truncate">{session?.email}</p>
                <span className="inline-block text-[9px] font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded mt-1.5 uppercase">
                  {session?.role || "USER"}
                </span>
              </div>
              <button
                onClick={handleLogout}
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 rounded-xl text-xs font-bold transition"
              >
      ...
      <aside className="hidden md:flex fixed top-0 left-0 w-64 h-full bg-white border-r border-slate-200 p-6 flex-col justify-between z-30">
        <div>
          {/* Brand header */}
        ...
        <div className="space-y-4 pt-6 border-t border-slate-100">
          <div className="px-2">
            <p className="text-xs font-bold text-slate-800 truncate">{session?.name || "User"}</p>
            <p className="text-[10px] text-slate-500 truncate">{session?.email}</p>
            <span className="inline-block text-[9px] font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded mt-1.5 uppercase">
              {session?.role || "USER"}
            </span>
          ...
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 hover:text-slate-950 rounded-xl text-xs font-bold transition active:scale-[0.98] shadow-sm"
          >
            <LogOut className="h-4 w-4" />
            Sign Out
          </button>
