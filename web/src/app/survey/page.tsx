...
export default function SurveyPage() {
  ...
  const submitSurvey = async () => {
    ...
    try {
      // Create request payload matching the database schema properties
      ...
      // Wrap in FormData for file upload
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between overflow-x-hidden relative">
      {/* Sticky Progress Bar */}
      <div className="fixed top-0 left-0 w-full z-50 bg-slate-900 h-2">
        <div 
      ...
      <main className="flex-grow flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-2xl bg-slate-900/60 border border-slate-800/80 shadow-2xl rounded-[2rem] p-6 sm:p-10 backdrop-blur-xl relative z-10 transition-all">
          ...
          {currentStep === 6 && (
            <section className="space-y-6 animate-fade-in">
              ...
              <div className="space-y-5">
                ...
                {[
                  { label: "Q1. Little interest or pleasure in doing things? *", key: "q" },
                  ...
                  const stateKey = index === 0 ? "q1" : index === 1 ? "q2" : index === 2 ? "q3" : "q4";
                  const val = (formData as any)[stateKey];
                  return (
                    <div key={stateKey} className="space-y-2 pt-2 border-t border-slate-800/40">
                      ...
                      <div className="grid grid-cols-4 gap-1.5">
                        {[
                          ...
                          <button
                            key={choice.val}
                            type="button"
                            onClick={() => updateField(stateKey, choice.val)}
      ...
      <footer className="w-full text-center py-6 text-xs text-slate-500">
        &copy; 2026 Multimodal Mental Health Screening Survey (Stateless Web Mode).
      </footer>
    </div>
  );
}
